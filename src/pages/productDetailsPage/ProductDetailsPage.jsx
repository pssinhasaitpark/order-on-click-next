...
//   const displayImages = product.images.length > 0 ? images : fallbackImages;
//   console.log("Display Images:", product.images.length);
...
const ProductDetailPage = () => {
  ...
  // Fix: use correct key `images` instead of `grocery_image`
  const images = product?.images?.map((img) => img.image_file) || [];
  ...
  // Fix: use `prices` instead of `grocery_price`
  const productPrice =
    product?.prices?.[0]?.discount || product?.prices?.[0]?.mrp_price || "0";
  const productMrpPrice = product?.prices?.[0]?.mrp_price || "0";
  const productCategory = product?.category_name || "Category";
  console.log("Product Data:", product);
  // Only render the page if product data is available
  if (!product) {
